...
let monacoViewStates = {}; // id -> monaco.editor.ICodeEditorViewState
...
function saveToStorage() {
  try {
    ...
    state.files.forEach(f => {
      if (state.activeId === f.id && state.monacoReady && monacoEditor) {
        const pos = monacoEditor.getPosition();
        if (pos) cursors[f.id] = { lineNumber: pos.lineNumber, column: pos.column };
      } else if (monacoViewStates[f.id]?.cursorState?.[0]?.position) {
        cursors[f.id] = monacoViewStates[f.id].cursorState[0].position;
      }
    });
...
function loadFromStorage() {
  try {
    ...
    if (typeof data.terminalOpen === 'boolean') state.terminalOpen = data.terminalOpen;
    if (typeof data.terminalHeight === 'number') state.terminalHeight = data.terminalHeight;
    state._savedCursors = data.cursors || {};
...
function registerCustomCompletions() {
  ...
  monaco.languages.registerCompletionItemProvider('java', {
    ...
    provideCompletionItems(model, position) {
      ...
      const range = { startLineNumber: position.lineNumber, endLineNumber: position.lineNumber, startColumn: word.startColumn, endColumn: word.endColumn };
      const snippets = [
        { label: 'sout',    insertText: 'System.out.println(${1:value});',                                         doc: 'Print to console' },
        ...
        { label: 'try',     insertText: 'try {\n\t$0\n} catch (${1:Exception} e) {\n\te.printStackTrace();\n}', doc: 'Try-catch' },
        { label: 'arr',     insertText: '${1:int}[] ${2:arr} = new ${1:int}[${3:size}];',                       doc: 'Array declaration' },
        { label: 'list',    insertText: 'List<${1:String}> ${2:list} = new ArrayList<>();',                      doc: 'ArrayList' },
        { label: 'map',     insertText: 'Map<${1:String}, ${2:Integer}> ${3:map} = new HashMap<>();',           doc: 'HashMap' },
      ];
      return { suggestions: snippets.map(s => ({
  ...
  monaco.languages.registerCompletionItemProvider('go', {
    ...
    provideCompletionItems(model, position) {
      ...
      const snippets = [
        ...
        { label: 'forrange',insertText: 'for ${1:i}, ${2:v} := range ${3:slice} {\n\t$0\n}',                  doc: 'For-range loop' },
        { label: 'if',      insertText: 'if ${1:condition} {\n\t$0\n}',                                        doc: 'If statement' },
        { label: 'iferr',   insertText: 'if err != nil {\n\treturn $0err\n}',                                  doc: 'Error check' },
        { label: 'struct',  insertText: 'type ${1:Name} struct {\n\t$0\n}',                                    doc: 'Struct definition' },
        { label: 'goroutine', insertText: 'go func() {\n\t$0\n}()',                                           doc: 'Goroutine' },
        { label: 'fmt',     insertText: 'fmt.Println(${1:value})',                                              doc: 'Print line' },
        { label: 'slice',   insertText: '${1:name} := make([]${2:int}, ${3:0})',                               doc: 'Make slice' },
        { label: 'map',     insertText: '${1:name} := make(map[${2:string}]${3:int})',                         doc: 'Make map' },
        { label: 'chan',     insertText: '${1:name} := make(chan ${2:int})',                                    doc: 'Make channel' },
      ];
      return { suggestions: snippets.map(s => ({
        label: s.label, kind: monaco.languages.CompletionItemKind.Snippet,
        insertText: s.insertText, insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
  ...
  monaco.languages.registerCompletionItemProvider('rust', {
    ...
    provideCompletionItems(model, position) {
      ...
      const snippets = [
        ...
        { label: 'vec',     insertText: 'let ${1:v}: Vec<${2:i32}> = Vec::new();',                           doc: 'Vector' },
        { label: 'println', insertText: 'println!("{}", ${1:value});',                                       doc: 'Print macro' },
        { label: 'let',     insertText: 'let ${1:name}: ${2:Type} = ${3:value};',                            doc: 'Let binding' },
...
function setActiveFile(id, animate = true) {
  ...
  if (oldId && oldId !== id && state.monacoReady && monacoEditor && monacoModels[oldId]) {
    monacoViewStates[oldId] = monacoEditor.saveViewState();
  }
  ...
  if (state.monacoReady && monacoEditor) {
    ...
    if (monacoViewStates[id]) {
      monacoEditor.restoreViewState(monacoViewStates[id]);
    } else if (state._savedCursors?.[id]) {
      monacoEditor.setPosition(state._savedCursors[id]);
      monacoEditor.revealLineInCenter(state._savedCursors[id].lineNumber);
    }
    monacoEditor.focus();
  }
  ...
  updateEditorStats();
  resetTimeline();
  _mentorContextKey = ''; // mentor context changes with file
...
async function saveToBackend() {
  try {
    ...
    if (res.ok) {
      ...
      console.log(`[CodeLens] Backend save OK — session ${_sessionId}`);
    }
  } catch (err) {
    // Backend unavailable — localStorage save already succeeded, so this is non-critical
    console.warn('[CodeLens] Backend save skipped (server unavailable):', err.message);
  }
}

// ── Backend explain (POST /api/explain) ──────────────────────
async function callExplainAPI(code, lang, question, aiMode) {
  try {
    const res = await fetch(`${API_BASE}/api/explain`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ code, lang, question, aiMode }),
...
function toggleTerminal() {
  ...
  const progInp = document.getElementById('program-input-panel');
  ...
  if (state.terminalOpen) {
    ...
    if (progInp) progInp.style.display = 'block';
    ...
    if (progInp) progInp.style.display = 'none';
  }
...
function terminalLog(type, msg) {
  ...
  if (msg === '') {
    ...
    let pref = prefix[type] || '$';
    if (/^(⚙️|✅|▶|❌|──|Exit Code|Execution Time|File |main\.|Traceback|Suggestion:|Did you forget|\^)/.test(msg)) {
      pref = '';
    }
    const prefSpan = pref ? `<span class="terminal-line-prefix" style="color:var(--accent-primary)">${pref}</span> ` : '';
    div.innerHTML = `${prefSpan}<span>${escHtml(msg)}</span>`;
    body.appendChild(div);
  }
...
// ── Code Execution via Local Runtime (/api/run) ───────────────
// ── Code Execution via Local Runtime (/api/run) ───────────────
function runCurrentCode() {
  ...
  // Ensure terminal pane is visible and clear output only when starting a new execution
  const pane = document.getElementById('terminal-pane');
  ...
  // Clear previous error markers when starting a run
  if (state.monacoReady && monacoEditor && monacoEditor.getModel()) {
    monaco.editor.setModelMarkers(monacoEditor.getModel(), 'run-errors', []);
  }

  // Save cursor position and scroll state before run
  const savedPosition = monacoEditor?.getPosition();
  const savedScrollTop = monacoEditor?.getScrollTop();

  state.isExecuting = true;
  if (currentRunAbortController) currentRunAbortController.abort();
  currentRunAbortController = new AbortController();

  const runBtn = document.getElementById('run-code-btn');
  const tlPlay = document.getElementById('tl-play');
  const oldRunText = runBtn ? runBtn.innerHTML : '▶ Run';
  const oldTlText = tlPlay ? tlPlay.innerHTML : '▶ Run';
  if (runBtn) { runBtn.disabled = true; runBtn.innerHTML = '<span class="spinner-border" style="width:12px;height:12px;border-width:2px;"></span> Running...'; }
  if (tlPlay) { tlPlay.disabled = true; tlPlay.innerHTML = '⏳ Running...'; }
  ...
  const stdinText = document.getElementById('program-input-textarea')?.value ?? '';
  fetch(`${API_BASE}/api/run`, {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify({ code, language: file.monacoLang || 'python', stdin: stdinText }),
    signal:  currentRunAbortController.signal
  })
  .then(r => r.json())
  .then(data => {
    if (state.monacoReady && monacoEditor) monacoEditor.updateOptions({ readOnly: false });

    const compileErr = data.compileError || data['compile errors'] || data.compileErrors || '';
    const runtimeErr = data.runtimeError || data['runtime errors'] || data.runtimeErrors || '';
    let stdout = data.stdout ?? data.output ?? '';
    const exitCode = data.exitCode ?? data['exit code'] ?? 0;

    // Requirement 3: Enforce safe display limits (10,000 lines or 1 MB) on frontend too
    const maxLines = 10000;
    const maxBytes = 1024 * 1024;
    if (stdout.length > maxBytes || stdout.split(/\r?\n/).length > maxLines) {
      stdout = stdout.split(/\r?\n/).slice(0, maxLines).join('\n').slice(0, maxBytes) + '\n\nOutput truncated because it exceeded the display limit.';
    }

    // Automatically scroll to, highlight line, and underline error column if available
    if (data.line && state.monacoReady && monacoEditor && monacoEditor.getModel()) {
      const model = monacoEditor.getModel();
      const lineNum = Number(data.line);
      const colNum = Number(data.column) || 1;
      const msg = data.message || compileErr || runtimeErr || 'Execution error';
      ...
      const marker = {
        ...
        endColumn: model.getLineMaxColumn(lineNum) || (colNum + 10),
        message: msg,
        severity: monaco.MarkerSeverity.Error
      };
      monaco.editor.setModelMarkers(model, 'run-errors', [marker]);
    } else if (exitCode === 0 && !compileErr && !runtimeErr) {
      if (state.monacoReady && monacoEditor && monacoEditor.getModel()) {
        monaco.editor.setModelMarkers(monacoEditor.getModel(), 'run-errors', []);
        if (savedPosition) monacoEditor.setPosition(savedPosition);
        if (savedScrollTop !== undefined) monacoEditor.setScrollTop(savedScrollTop);
      }
    }

    if (compileErr && compileErr.trim()) {
      terminalLog('error', 'Compilation Error');
      terminalLog('info', '--------------------');
      compileErr.split('\n').forEach(line => terminalLog('error', line));
      terminalLog('info', '--------------------');
      terminalLog('info', `Exit Code: ${exitCode}`);
      if (data.executionTime !== undefined) {
        terminalLog('info', `Execution Time: ${data.executionTime} ms`);
      }
    } else {
      if (isCompileLang) {
        terminalLog('info', 'Running...');
      ...
      } else if (!runtimeErr && !compileErr) {
        terminalLog('output', '(no output)');
      }
      if (runtimeErr && runtimeErr.trim()) {
        if (stdout && stdout.trim()) terminalLog('info', '');
        runtimeErr.split('\n').forEach(line => terminalLog('error', line));
      }
      terminalLog('info', '--------------------');
      const isSuccess = exitCode === 0 && !runtimeErr && !compileErr;
      terminalLog(isSuccess ? 'success' : 'info', `Exit Code: ${exitCode}`);
      if (data.executionTime !== undefined) {
        terminalLog(isSuccess ? 'success' : 'info', `Execution Time: ${data.executionTime} ms`);
      }
    }

    // Show success briefly before restoring Run button on successful run
    const isSuccessRun = exitCode === 0 && !compileErr && !runtimeErr;
    if (isSuccessRun) {
      if (runBtn) runBtn.innerHTML = '✅ Success';
      if (tlPlay) tlPlay.innerHTML = '✅ Success';
      setTimeout(() => {
        if (!state.isExecuting) {
          if (runBtn) { runBtn.disabled = false; runBtn.innerHTML = oldRunText; }
          if (tlPlay) { tlPlay.disabled = false; tlPlay.innerHTML = oldTlText; }
        }
      }, 1000);
    } else {
      if (runBtn) { runBtn.disabled = false; runBtn.innerHTML = oldRunText; }
    ...
    if (state.monacoReady && monacoEditor) monacoEditor.focus();
  })
  .catch(err => {
    state.isExecuting = false;
    currentRunAbortController = null;
    ...
    if (err.name !== 'AbortError') {
      terminalLog('error', 'Runtime Error');
      terminalLog('info', '--------------------');
      terminalLog('error', err.message || 'Network failure / backend unavailable.');
      terminalLog('info', '--------------------');
      terminalLog('info', 'Exit Code: 1');
    }
    if (state.monacoReady && monacoEditor) monacoEditor.focus();
  });
}
...
document.querySelectorAll('.ai-mode-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    updateAiModeLabel();
    state.conversation = [];
    _mentorContextKey = ''; // force mentor panel to re-initialise with new mode
  });
});
...
async function analyzeCode(file) {
  ...
  // Always pull latest content from Monaco
  const code = (state.monacoReady && monacoEditor) ? monacoEditor.getValue() : file.content;
  if (!code.trim()) {
  ...
  // Build local analysis for the execution timeline (still used by Visualizer)
  try {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/explain`, {
      ...
      signal:  AbortSignal.timeout(45000), // 45 s for large files
    });
    ...
    if (err.name === 'TimeoutError' || err.name === 'AbortError') {
      ...
      msg = 'Cannot reach the server.\nIs it running? Start it with: npm run dev';
    }
    renderAiError(msg);
    setAiStatus('Error', 'var(--accent-rose)');
...
function renderExplainPanel(file, a) {
  ...
  renderAiContent(`
    ...
    <div class="ai-explanation-block">
      <div class="ai-block-label">🧩 Structure</div>
      <div class="ai-block-text">
        <div style="margin-bottom:8px"><span style="font-size:11px;color:var(--text-muted)">FUNCTIONS</span><br/>${fnList}</div>
        <div style="margin-bottom:8px"><span style="font-size:11px;color:var(--text-muted)">CLASSES</span><br/>${clsList}</div>
        <div><span style="font-size:11px;color:var(--text-muted)">IMPORTS</span><br/>${impList}</div>
      </div>
    </div>
...
// ── Groq AI Explain Panel — Collapsible Cards ──────────────
function renderGroqExplainPanel(file, data) {
  // Safe accessors
  const safeStr = v => (v != null && typeof v !== 'object') ? String(v) : (v ? JSON.stringify(v) : '');
  const safeArr = v => Array.isArray(v) ? v : (v ? [safeStr(v)] : []);

  // Complexity colours
  const timeCx  = safeStr(data.timeComplexity  || '');
  const spaceCx = safeStr(data.spaceComplexity || '');
  const timeCol  = /2.n|2\^|exp/i.test(timeCx)  ? 'var(--accent-rose)'
                 : /n.2|n\^2|quadr/i.test(timeCx) ? 'var(--accent-amber)'
                 : 'var(--accent-emerald)';
  const spaceCol = /O\(1\)/.test(spaceCx) ? 'var(--accent-emerald)' : 'var(--accent-amber)';

  // ── Card builder helper ────────────────────────────────────
  // startOpen = false by default; only Summary and Output default to true
  function card(icon, title, badge, accentColor, bodyHtml, startOpen = false) {
    if (!bodyHtml || !bodyHtml.trim()) return '';
    const headerColor = accentColor || 'var(--accent-primary)';
    const borderStyle = accentColor ? `border-color:${accentColor.replace('var(--accent-','rgba(').replace(')',', 0.2)')};` : '';
    return `
      <div class="ai-explanation-block" style="${borderStyle}">
        <div class="ai-card-header${startOpen ? '' : ' collapsed'}" role="button" tabindex="0" aria-expanded="${startOpen}">
          <span class="ai-card-icon">${icon}</span>
          <span class="ai-card-title" style="color:${headerColor}">${title}</span>
          ${badge ? `<span class="ai-card-badge" style="color:${headerColor};background:${headerColor.replace('var(--accent-','rgba(').replace(')',', 0.1)')};border-color:${headerColor.replace('var(--accent-','rgba(').replace(')',', 0.25)')};">${badge}</span>` : ''}
          <span class="ai-card-chevron">▾</span>
        </div>
        <div class="ai-card-body-wrapper${startOpen ? '' : ' collapsed'}">
          <div class="ai-card-body">
            <div class="ai-card-content">${bodyHtml}</div>
          </div>
        </div>
      </div>`;
  }

  // ── 1. Summary (Default: Expanded) ────────────────────────
  const summaryHtml = data.summary ? card('🧠', 'Summary', 'Groq AI', 'var(--accent-primary)',
    `<p style="font-size:13px;color:var(--text-secondary);line-height:1.7;margin:0">${escHtml(safeStr(data.summary))}</p>`,
    true
  ) : '';

  // ── 2. Output (Default: Expanded) ─────────────────────────
  const outputHtml = data.output ? card('📤', 'Output', 'Expected', 'var(--accent-emerald)',
    `<div style="font-family:var(--font-mono);font-size:12px;color:var(--accent-emerald);background:var(--bg-overlay);padding:10px 12px;border-radius:6px;border:1px solid rgba(16,185,129,0.2)">${escHtml(safeStr(data.output))}</div>`,
    true
  ) : '';

  // ── 3. Complexity (Default: Collapsed) ────────────────────
  const complexityBody = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
      <div style="background:var(--bg-overlay);border-radius:8px;padding:12px;text-align:center;border:1px solid var(--border-subtle)">
        <div style="font-size:10px;color:var(--text-muted);font-weight:600;letter-spacing:.08em;text-transform:uppercase;margin-bottom:6px">TIME</div>
        <div style="font-family:var(--font-mono);font-weight:700;font-size:18px;color:${timeCol};margin-bottom:4px">${escHtml(timeCx.split(' ')[0] || '—')}</div>
        ${timeCx.includes(' ') ? `<div style="font-size:11px;color:var(--text-muted)">${escHtml(timeCx.split(' ').slice(1).join(' '))}</div>` : ''}
      </div>
      <div style="background:var(--bg-overlay);border-radius:8px;padding:12px;text-align:center;border:1px solid var(--border-subtle)">
        <div style="font-size:10px;color:var(--text-muted);font-weight:600;letter-spacing:.08em;text-transform:uppercase;margin-bottom:6px">SPACE</div>
        <div style="font-family:var(--font-mono);font-weight:700;font-size:18px;color:${spaceCol};margin-bottom:4px">${escHtml(spaceCx.split(' ')[0] || '—')}</div>
        ${spaceCx.includes(' ') ? `<div style="font-size:11px;color:var(--text-muted)">${escHtml(spaceCx.split(' ').slice(1).join(' '))}</div>` : ''}
      </div>
    </div>`;
  const complexityHtml = (timeCx || spaceCx) ? card('⚡', 'Complexity', '', null, complexityBody, false) : '';

  // ── 4. Algorithm (Default: Collapsed) ─────────────────────
  const algorithmHtml = data.algorithm ? card('⚙', 'Algorithm', '', 'var(--accent-cyan)',
    `<p style="font-size:13px;color:var(--text-secondary);line-height:1.7;margin:0">${escHtml(safeStr(data.algorithm))}</p>`,
    false
  ) : '';

  // ── 5. Line-by-line (Default: Collapsed) ──────────────────
  const lines = safeArr(data.lineByLine);
  const lineBody = lines.length ? `
    <div style="display:flex;flex-direction:column;gap:5px">
      ${lines.slice(0, 20).map(l => `
        <div style="display:grid;grid-template-columns:36px 1fr;gap:8px;align-items:start;padding:7px 10px;background:var(--bg-overlay);border-radius:6px;border:1px solid var(--border-subtle)">
          <span style="font-family:var(--font-mono);font-size:10px;color:var(--accent-cyan);font-weight:700;padding-top:2px">L${escHtml(safeStr(l.line || ''))}</span>
          <div>
            ${l.code ? `<div style="font-family:var(--font-mono);font-size:11px;color:var(--text-primary);margin-bottom:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(safeStr(l.code).substring(0, 75))}</div>` : ''}
            <div style="font-size:12px;color:var(--text-secondary);line-height:1.6">${escHtml(safeStr(l.explanation))}</div>
          </div>
        </div>`).join('')}
      ${lines.length > 20 ? `<div style="font-size:11px;color:var(--text-muted);text-align:center;padding:4px">… and ${lines.length - 20} more lines</div>` : ''}
    </div>` : '';
  const lineHtml = lineBody ? card('📝', 'Line-by-Line', `${lines.length} lines`, null, lineBody, false) : '';

  // ── 6. Variables (Default: Collapsed) ─────────────────────
  const vars = safeArr(data.variables).slice(0, 15);
  const varsBody = vars.length ? `
    <table class="var-table">
      <thead><tr><th>Name</th><th>Type</th><th>Purpose</th></tr></thead>
      <tbody>${vars.map(v => `
        <tr>
          <td><span class="var-name">${escHtml(safeStr(v.name))}</span></td>
          <td><span class="var-type-badge">${escHtml(safeStr(v.type) || 'auto')}</span></td>
          <td style="font-size:12px;color:var(--text-secondary);font-family:var(--font-sans,inherit)">${escHtml(safeStr(v.purpose || v.description || '—'))}</td>
        </tr>`).join('')}
      </tbody>
    </table>` : '';
  const varsHtml = varsBody ? card('📋', 'Variables', `${vars.length}`, null, varsBody, false) : '';

  // ── 7. Functions (Default: Collapsed) ─────────────────────
  const fns = safeArr(data.functions).slice(0, 10);
  const fnsBody = fns.length ? `
    <div style="display:flex;flex-direction:column;gap:8px">
      ${fns.map(f => `
        <div style="padding:10px 12px;background:var(--bg-overlay);border-radius:8px;border:1px solid var(--border-subtle)">
          <div style="font-family:var(--font-mono);font-size:13px;color:var(--accent-blue);font-weight:700;margin-bottom:4px">
            ${escHtml(safeStr(f.name))}(${safeArr(f.params).map(p => escHtml(safeStr(p))).join(', ')})
            ${f.returns ? `<span style="font-size:11px;color:var(--accent-cyan);font-weight:400"> → ${escHtml(safeStr(f.returns))}</span>` : ''}
          </div>
          ${(f.purpose || f.description) ? `<div style="font-size:12px;color:var(--text-secondary);line-height:1.6">${escHtml(safeStr(f.purpose || f.description))}</div>` : ''}
        </div>`).join('')}
    </div>` : '';
  const fnsHtml = fnsBody ? card('🧩', 'Functions', `${fns.length}`, 'var(--accent-blue)', fnsBody, false) : '';

  // ── 8. Dry Run (Default: Collapsed) ───────────────────────
  const dryRunBody = data.dryRun ? `
    <div style="font-family:var(--font-mono);font-size:12px;color:var(--text-secondary);line-height:1.9;white-space:pre-wrap;background:var(--bg-overlay);padding:12px 14px;border-radius:6px;border:1px solid var(--border-subtle);max-height:260px;overflow-y:auto">${escHtml(safeStr(data.dryRun).substring(0, 1800))}</div>` : '';
  const dryRunHtml = dryRunBody ? card('🔍', 'Dry Run', 'Trace', 'var(--accent-amber)', dryRunBody, false) : '';

  // ── 9. Common Mistakes (Default: Collapsed) ───────────────
  const mistakes = safeArr(data.commonMistakes).slice(0, 8);
  const mistakesBody = mistakes.length ? `
    <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:7px">
      ${mistakes.map(m => `
        <li style="display:flex;gap:8px;align-items:flex-start;padding:8px 10px;background:rgba(244,63,94,0.05);border:1px solid rgba(244,63,94,0.15);border-radius:6px">
          <span style="color:var(--accent-rose);flex-shrink:0;margin-top:1px">⚠</span>
          <span style="font-size:12px;color:var(--text-secondary);line-height:1.6">${escHtml(safeStr(m))}</span>
        </li>`).join('')}
    </ul>` : '';
  const mistakesHtml = mistakesBody ? card('⚠', 'Common Mistakes', `${mistakes.length}`, 'var(--accent-rose)', mistakesBody, false) : '';

  // ── 10. Better Approach (Default: Collapsed) ──────────────
  const betterApproachHtml = data.betterApproach ? card('🚀', 'Better Approach', 'Optimized', 'var(--accent-emerald)',
    `<p style="font-size:13px;color:var(--text-secondary);line-height:1.7;margin:0">${escHtml(safeStr(data.betterApproach))}</p>`,
    false
  ) : '';

  // ── 11. Suggestions (Default: Collapsed) ──────────────────
  const suggestions = safeArr(data.suggestions).slice(0, 8);
  const suggestionsBody = suggestions.length ? `
    <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:6px">
      ${suggestions.map((s, i) => `
        <li style="display:flex;gap:8px;align-items:flex-start;padding:7px 10px;background:rgba(16,185,129,0.05);border:1px solid rgba(16,185,129,0.15);border-radius:6px">
          <span style="color:var(--accent-emerald);font-family:var(--font-mono);font-size:10px;font-weight:700;flex-shrink:0;margin-top:2px">${i+1}.</span>
          <span style="font-size:12px;color:var(--text-secondary);line-height:1.6">${escHtml(safeStr(s))}</span>
        </li>`).join('')}
    </ul>` : '';
  const suggestionsHtml = suggestionsBody ? card('💡', 'Suggestions', `${suggestions.length}`, 'var(--accent-emerald)', suggestionsBody, false) : '';

  // ── 12. Related Concepts (Default: Collapsed) ─────────────
  const concepts = safeArr(data.relatedConcepts).slice(0, 12);
  const conceptsBody = concepts.length ? `
    <div style="display:flex;flex-wrap:wrap;gap:6px">
      ${concepts.map(c => `
        <span style="padding:4px 10px;background:rgba(124,110,232,0.1);border:1px solid rgba(124,110,232,0.2);border-radius:99px;font-size:12px;color:var(--accent-primary)">${escHtml(safeStr(c))}</span>`).join('')}
    </div>` : '';
  const conceptsHtml = conceptsBody ? card('🔗', 'Related Concepts', `${concepts.length}`, 'var(--accent-primary)', conceptsBody, false) : '';

  // ── 13. Interview Questions (Default: Collapsed) ──────────
  const iqList = safeArr(data.interviewQuestions).slice(0, 8);
  const iqBody = iqList.length ? `
    <div style="display:flex;flex-direction:column;gap:6px">
      ${iqList.map((q, i) => `
        <div style="padding:9px 12px;background:var(--bg-overlay);border-radius:6px;border:1px solid var(--border-subtle)">
          <div style="display:flex;gap:8px;align-items:flex-start">
            <span style="color:var(--accent-primary);font-weight:700;font-family:var(--font-mono);font-size:10px;flex-shrink:0;margin-top:1px">Q${i+1}</span>
            <span style="font-size:12px;color:var(--text-secondary);line-height:1.6">${escHtml(safeStr(q))}</span>
          </div>
        </div>`).join('')}
    </div>` : '';
  const iqHtml = iqBody ? card('🎯', 'Interview Questions', `${iqList.length}`, 'var(--accent-primary)', iqBody, false) : '';
  ...
  renderAiContent(`
    ...
    ${conceptsHtml}
    ${iqHtml}
    <div style="font-size:11px;color:var(--text-muted);text-align:center;padding:4px 0 16px;display:flex;align-items:center;justify-content:center;gap:6px">
      <span style="width:6px;height:6px;border-radius:50%;background:var(--accent-emerald);display:inline-block"></span>
      Powered by Groq AI · llama-3.3-70b · Ask follow-up questions below ↓
    </div>
  `);

  // ── Wire up collapsible toggles ───────────────────────────
  // (Must run after DOM is updated by renderAiContent)
  requestAnimationFrame(() => {
    document.querySelectorAll('#ai-content .ai-card-header').forEach(header => {
...
// ── Visualization Panel ─────────────────────────────────
// Keep a single engine instance per container to avoid re-creation flicker
let _vizEngine = null;
...
function renderVizPanel(file) {
  if (window.Visualizer) {
    ...
    // Build exec steps if we don't have any for the current file
    if (!state.execSteps.length) {
      state.execSteps = window.Visualizer.buildExecutionTimeline(file.content, file.monacoLang, analysis);
      state.execIdx   = 0;
    }

    const execState = { stepIdx: state.execIdx, steps: state.execSteps };
    const container = document.getElementById('ai-content');
    if (!container) return;

    // Reuse existing engine if same container, otherwise create fresh
    if (!_vizEngine || _vizEngine.container !== container) {
      container.innerHTML = '';
      _vizEngine = new window.Visualizer.VisualizationEngine(container);
    }
    _vizEngine.render(file, analysis, execState);
  } else {
...
// ── Mentor Panel ──────────────────────────────
// Track which file/mode the mentor was last initialised for
let _mentorContextKey = '';

function renderMentorPanel(forceReset = false) {
  switchAiTab('mentor');
  const file = getActiveFile();
  const contextKey = `${file?.id || 'none'}_${state.aiMode}`;

  // Only re-render if context changed or forced (avoids unnecessary resets mid-conversation)
  const existing = document.getElementById('mentor-messages');
  if (existing && contextKey === _mentorContextKey && !forceReset) return;

  // Reset conversation when context changes
  if (contextKey !== _mentorContextKey) {
    state.conversation = [];
    _mentorIdx = 0;
  }
  _mentorContextKey = contextKey;

  const fname = file?.name || 'your code';
  const intros = {
    mentor:    `Hey! I can see you're working on <strong>${fname}</strong>. Before I explain anything — <strong style="color:var(--accent-primary)">what do you think this code does?</strong> Take a guess! 💡`,
...
function handleMentorSend(msg) {
  ...
  setTimeout(() => {
    ...
    // Show typing indicator immediately; actual reply comes from Groq async
    const file     = getActiveFile();
    ...
    // ── Call Groq mentor API (with local fallback) ──────────
    const replyPromise = window.AIEngine?.sendMentorMessage
      ? window.AIEngine.sendMentorMessage(msg, file, analysis, state.aiMode, state.conversation)
      : Promise.resolve(
          window.AIEngine
            ? window.AIEngine.generateAIResponse(msg, file, analysis, state.aiMode, state.conversation)
            : (mentorPools[state.aiMode] || mentorPools.mentor)[_mentorIdx++ % 5](file, msg)
        );
    ...
    replyPromise.then(reply => {
      ...
      // Strip raw HTML from local replies that return HTML string to avoid double-escaping
      const safeReply = typeof reply === 'string' ? reply : escHtml(String(reply));

      state.conversation.push({ role: 'assistant', content: safeReply });

      const messagesEl = document.getElementById('mentor-messages');
      if (messagesEl) {
        messagesEl.insertAdjacentHTML('beforeend', `
          <div class="mentor-msg" style="animation:fade-up .3s ease both">
            <div class="mentor-avatar">🔭</div>
            <div class="mentor-bubble">${safeReply}</div>
          </div>`);
      }
      if (content) content.scrollTop = content.scrollHeight;
    }).catch(err => {
      ...
      if (messagesEl) {
        messagesEl.insertAdjacentHTML('beforeend', `
          <div class="mentor-msg" style="animation:fade-up .3s ease both">
            <div class="mentor-avatar">🔭</div>
            <div class="mentor-bubble" style="color:var(--accent-rose)">⚠ Could not reach AI: ${escHtml(err.message)}</div>
          </div>`);
      }
      console.error('[CodeLens] Mentor error:', err);
    });
...
// ── Program Input Panel Toggle & Clear ─────────
const progTextarea = document.getElementById('program-input-textarea');
if (progTextarea) {
  progTextarea.value = localStorage.getItem('codelens_program_input') || '';
  progTextarea.addEventListener('input', () => {
    localStorage.setItem('codelens_program_input', progTextarea.value);
  });
}
document.getElementById('program-input-toggle-header')?.addEventListener('click', e => {
  if (e.target.id === 'program-input-clear-btn' || e.target.closest('#program-input-clear-btn')) return;
  const body = document.getElementById('program-input-body');
  const label = document.getElementById('program-input-toggle-label');
  if (body && label) {
    const isHidden = body.style.display === 'none';
    body.style.display = isHidden ? 'block' : 'none';
    label.textContent = isHidden ? '▼ Program Input (stdin)' : '▶ Program Input (stdin)';
  }
});
document.getElementById('program-input-clear-btn')?.addEventListener('click', e => {
  e.stopPropagation();
  const textarea = document.getElementById('program-input-textarea');
  if (textarea) {
    textarea.value = '';
    localStorage.removeItem('codelens_program_input');
  }
});
...
// Cache the welcome HTML before anything can replace it
const _welcomeHtml = document.getElementById('ai-welcome')?.outerHTML || `
  <div class="ai-welcome" id="ai-welcome">
    <div style="font-size:36px;margin-bottom:12px">🧠</div>
    <div style="font-size:14px;font-weight:600;margin-bottom:8px">AI ready</div>
    <div style="font-size:13px;color:var(--text-muted);line-height:1.7;max-width:240px;margin:0 auto">Create a file, write your code, then click <strong style="color:var(--accent-primary)">Analyze</strong>.</div>
  </div>`;
...
document.getElementById('ai-clear-btn')?.addEventListener('click', () => {
  ...
  _mentorIdx = 0;
  _mentorContextKey = '';
  renderAiContent(_welcomeHtml);
  switchAiTab('explain');
  if (window.AIEngine?.showToast) window.AIEngine.showToast('Chat cleared', 'info', 1800);
});
...
// Store decoration IDs per Monaco model so we can clear them properly
const _execDecorations = {}; // modelId -> decorationId[]
...
function highlightLine() {
  ...
  // Clear previous decorations for this model, apply new ones
  const modelId = model.id || 'default';
  const prevIds = _execDecorations[modelId] || [];
  const newIds  = monacoEditor.deltaDecorations(
    prevIds,
    [{
  ...
  _execDecorations[modelId] = newIds;
}
...
// ══════════════════════════════════════════════
const COMMANDS = [
  { icon:'▶',  label:'Run Code',              sub:'Execute file via Local Runtime (Ctrl+Enter)', action: runCurrentCode,                     kbd:['Ctrl','Enter'] },
...
document.addEventListener('keydown', e => {
  ...
  if (mod && e.key === 'Enter') { e.preventDefault(); runCurrentCode(); }
